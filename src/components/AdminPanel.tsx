...
const AdminPanel: React.FC = () => {
  ...
  const handleUpdateStatus = async (userId: string, targetStatus: AppUser['status']) => {
    ...
    if (userToUpdate?.email?.toLowerCase() === ADMIN_MASTER_EMAIL) {
      alert("Ação Bloqueada: O Administrador Master deve permanecer sempre ativo.");
      return;
    }
  ...
  const handleDeleteTrigger = (e: React.MouseEvent, user: AppUser) => {
    ...
    if (user.email?.toLowerCase() === ADMIN_MASTER_EMAIL) {
      alert("Ação Bloqueada: O Administrador Master jamais pode ser removido do sistema.");
      return;
    }
  ...
  return (
    <div className="glass p-4 md:p-6 rounded-[32px] border border-white/5 shadow-2xl text-white animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="space-y-3">
        {filteredUsers.map(user => {
          ...
          return (
            <div key={user.user_id} className={`bg-white/5 border p-3 md:p-4 rounded-[24px] flex flex-col gap-3 group transition-all ${isMaster ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-white/5'}`}>
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] italic mt-0.5">ID: {user.user_id.slice(0, 10)}...</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 w-full lg:w-auto items-center">
                  {!isMasterEmail ? (
                    <>
                      {user.status !== 'ativo' && (
                        <button onClick={() => handleUpdateStatus(user.user_id, 'ativo')} className="flex-1 lg:flex-none px-4 py-2 h-9 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-black uppercase italic rounded-lg transition-all shadow-lg shadow-emerald-900/20">APROVAR</button>
                      )}
                      {user.status !== 'pausado' && (
                        <button onClick={() => handleUpdateStatus(user.user_id, 'pausado')} className="flex-1 lg:flex-none px-4 py-2 h-9 bg-amber-600 hover:bg-amber-500 text-white text-[10px] font-black uppercase italic rounded-lg transition-all shadow-lg shadow-amber-900/20">PAUSAR</button>
                      )}
                      {user.status !== 'negado' && (
                        <button onClick={() => handleUpdateStatus(user.user_id, 'negado')} className="flex-1 lg:flex-none px-4 py-2 h-9 bg-orange-600 hover:bg-orange-500 text-white text-[10px] font-black uppercase italic rounded-lg transition-all shadow-lg shadow-orange-900/20">NEGAR</button>
                      )}
                      {user.status !== 'bloqueado' && (
                        <button onClick={() => handleUpdateStatus(user.user_id, 'bloqueado')} className="flex-1 lg:flex-none px-4 py-2 h-9 bg-red-700 hover:bg-red-600 text-white text-[10px] font-black uppercase italic rounded-lg transition-all shadow-lg shadow-red-900/20">BLOQUEAR</button>
                      )}
            ...
            <div className="flex gap-4">
              ...
              <button 
                ...
                onClick={async () => {
                  ...
                  await supabaseService.updateProfileStatus(userToDelete.user_id, 'pausado');
                  setUserToDelete(null);
                  loadAdminData();
                }}
